  ...
  constructor(workgroupSize: number, label: string, device: GPUDevice, gridSize: number[]) {
    super(
      /* wgsl */ `
        ...
        @group(1) @binding(0) var<storage, read> particlesPast: array<Particle>;
        ...
        @group(2) @binding(1) var<storage, read_write> grid: array<u32, ${(gridSize[0] * gridSize[1])}>;

        //var<workgroup> grid: array<atomic<u32>, ${(gridSize[0] * gridSize[1])}>;
        ...
        @compute @workgroup_size(${workgroupSize}) fn c0(
            ...
            let pp = particlesPast[global_invocation_index];

            let gridCoords = vec2i(ceil(pp.position / f32(globals.gridCellSizeInPixels)));
            particles[global_invocation_index].gridCoords = gridCoords;
            
            let index = getIndexFromGridPos(gridCoords);
            
            
            if(grid[index] == 0){
                grid[index] = global_invocation_index + 1;
            }

            //atomicStore(&grid[index], global_invocation_index + 1);
            //workgroupBarrier();
